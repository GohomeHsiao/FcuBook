...
if($_POST[control_del] == 1){
	...
	for($i=0; $i< sizeof($del_list); $i++){
		...
		$sql = "DELETE FROM ask_ans WHERE ask_no = ".$del_list[$i];		
		mysql_query($sql, $link);
	}	
}
//處理刪除END
...
$sql = "SELECT * FROM ask WHERE EXISTS (SELECT * FROM ask_ans WHERE ask.ask_no = ask_ans.ask_no)";
...
//處理中間列表START
$sql = "SELECT * FROM ask JOIN ask_ans ON ask.ask_no = ask_ans.ask_no WHERE EXISTS (SELECT * FROM ask_ans WHERE ask.ask_no = ask_ans.ask_no) ORDER BY ask_time DESC LIMIT $start, $pens_per_page";
$tmp = mysql_query($sql, $link);
$ask_list = array();
...
<div id="main" class="main">
	<p class="line30T">
	客戶提問管理 - 已回答</p>
	<form method="POST" name="ask_form" action="qa01_2.php">
		<table border="1" id="table1" width="650">
			...
			<tr>
				...
				<td class="tdT" width="320" align="center">主題</td>
				<td class="tdT" width="75" align="center">發問者</td>
				<td class="tdT" width="80" align="center">發問時間</td>
				<td class="tdT" width="80" align="center">回答時間</td>
			</tr>
			<?php
			for($i=0; $i< sizeof($ask_list); $i++){
				...
				echo '<td width="320" align="center" '; if($i%2 == 1) echo 'bgcolor="#DFDFDF"'; echo '><a href="qa_answered.php?ask_no='.$ask_list[$i][ask_no].'">'.$ask_list[$i][subject].'</a></td>';
				echo '<td width="75" align="center" '; if($i%2 == 1) echo 'bgcolor="#DFDFDF"'; echo '>'.$ask_list[$i][asker].'</td>';
				echo '<td width="80" align="center" '; if($i%2 == 1) echo 'bgcolor="#DFDFDF"'; echo '>'.$ask_list[$i][ask_time].'</td>';
				echo '<td width="80" align="center" '; if($i%2 == 1) echo 'bgcolor="#DFDFDF"'; echo '>'.$ask_list[$i][ans_time].'</td>';
				echo '</tr>';
			}
			?>
			<tr>
